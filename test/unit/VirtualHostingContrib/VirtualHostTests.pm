...
sub create_vhost {
    ...
    return Foswiki::Contrib::VirtualHostingContrib::VirtualHost->find(
        $hostname);
}
...
sub _dump_config {
    ...
    if ( ref( $config{$key} ) eq 'HASH' ) {
        join( '',
            map { _dump_config( "${prefix}{$key}", $_, %{ $config{$key} } ) }
              keys( %{ $config{$key} } ) );
    }
    else {
...
sub test_existing_vhost {
    ...
    my $vhost =
      Foswiki::Contrib::VirtualHostingContrib::VirtualHost->find('lala.com');
...
sub test_unexisting_vhost {
    my $this  = shift;
    my $vhost = Foswiki::Contrib::VirtualHostingContrib::VirtualHost->find(
        'unexisting.com');
    $this->assert( !$vhost,
        "searching for an unexisting virtualhost must return nothing" );
}
...
sub test_find_with_invalid_hostname {
    ...
    my $vhost = Foswiki::Contrib::VirtualHostingContrib::VirtualHost->find(
        '../../../../../../etc/passwd');
    $this->assert( !defined $vhost,
        "trying invalid hostname must return no virtualhost at all" );
}
...
sub test_validate_hostanme {
    ...
    for my $valid (@valid_list) {
        $this->assert(
            Foswiki::Contrib::VirtualHostingContrib::VirtualHost::_validate_hostname(
                $valid) eq $valid,
            "$valid must be valid"
        );
    }
...
sub test_local_setting_of_variables {
    ...
    foreach
      my $config (qw(DataDir PubDir WorkingDir DefaultUrlHost TemplatePath))
    {
        my $default = $Foswiki::cfg{$config};
        my $inside = $vhost->run( sub { $Foswiki::cfg{$config} } );
        $this->assert( defined $inside,
            "must define $config inside virtual host" );
        $this->assert(
            $inside ne $default,
"must change $config when running inside virtual host (default: $default)"
        );
    }
}
...
sub test_modified_TemplatePath {
    ...
    my $modified_TemplatePath =
      $vhost->run( sub { $Foswiki::cfg{TemplatePath} } );

    my @modified_path = split( /\s*,\s*/, $modified_TemplatePath );
    for my $component ( split( /\s*,\s*/, $original_TemplatePath ) ) {
        $this->assert( grep { $_ eq $component } @modified_path,
            "component $component from original TemplatePath must be kept" );
    }
}
...
sub test_virtualhost_template_path {
    ...
    $this->assert(
        grep { $_ eq "$ROOT/example.com/templates/\$web/\$name.\$skin.tmpl" }
          @path,
        'must add virtualhost templates to TemplatePath (web/topic)'
    );
    $this->assert(
        grep { $_ eq "$ROOT/example.com/templates/\$name.\$skin.tmpl" } @path,
        'must add virtualhost templates to TemplatePath (name/skin)'
    );
    $this->assert(
        grep { $_ eq "$ROOT/example.com/templates/\$web/\$name.tmpl" } @path,
        'must add virtualhost templates to TemplatePath (web/skin)'
    );
    $this->assert(
        grep { $_ eq "$ROOT/example.com/templates/\$name.tmpl" } @path,
        'must add virtualhost templates to TemplatePath (name)'
    );
}
...
sub test_custom_WebNames {
    ...
    for my $config ( 'SystemWebName', 'UsersWebName', 'TrashWebName' ) {
        ...
        $this->assert_str_equals( $orig, $vhost_value,
...
sub test_reading_config_file_with_Foswiki_cfg_reference {
    my $this = shift;
    create_config_with_content( 'example.com',
        '$Foswiki::cfg{SomeSetting} = "SomeValue"; 1;' );
    my $vhost = create_vhost('example.com');
    $this->assert_str_equals( 'SomeValue',
        $vhost->run( sub { $Foswiki::cfg{'SomeSetting'}; } ) );
...
sub test_run_on_each {
    ...
    $this->assert_deep_equals( \@expected, \@list,
        'must run a block inside each virtual host' );
}
...
sub test_run_sets_CURRENT {
    my $this    = shift;
    my $vhost   = create_vhost('example.com');
    my $current = $vhost->run(
        sub { $Foswiki::Contrib::VirtualHostingContrib::VirtualHost::CURRENT }
    );
...
sub test_redefine_JSCalendarContrib_style_configuration {
    ...
    my $vhost = create_vhost('example.com');
    my $vhost_value =
      $vhost->run( sub { $Foswiki::cfg{JSCalendarContrib}->{style} } );
    $this->assert_equals( $orig, $vhost_value, "use default when not set" );

    # set
    create_config( 'paulofreire.org',
        JSCalendarContrib => { style => 'brown' } );
    my $vhost2 = create_vhost('paulofreire.org');
    my $jscalendarcontrib_value =
      $vhost2->run( sub { $Foswiki::cfg{JSCalendarContrib}->{style} } );
    $this->assert_str_equals( 'brown', $jscalendarcontrib_value,
        "setting JSCalendarContrib style" );
}
...
sub test_redefine_JSCalendarContrib_lang_configuration {
    ...
    my $vhost = create_vhost('example.com');
    my $vhost_value =
      $vhost->run( sub { $Foswiki::cfg{JSCalendarContrib}->{lang} } );
    $this->assert_equals( $orig, $vhost_value, "use default when not set" );
    ...
    my $vhost2 = create_vhost('paulofreire.org');
    my $jscalendarcontrib_value =
      $vhost2->run( sub { $Foswiki::cfg{JSCalendarContrib}->{lang} } );
    $this->assert_str_equals( 'pt', $jscalendarcontrib_value,
        "setting JSCalendarContrib lang" );
}
...
sub test_set_arbitrary_configurations {
    ...
    my $vhost =
      Foswiki::Contrib::VirtualHostingContrib::VirtualHost->find('example.com');
    $this->assert_str_equals( 'NewValue',
        $vhost->run( sub { $Foswiki::cfg{SomeSetting} } ) );
}
...
sub test_set_arbitrary_configurations_with_hashes {
    ...
    create_config( 'example.com',
        'SomePlugin' => { Setting1 => 'Value1', Setting2 => 'Value2' } );

    my $vhost =
      Foswiki::Contrib::VirtualHostingContrib::VirtualHost->find('example.com');

    $this->assert_str_equals( 'Value1',
        $vhost->run( sub { $Foswiki::cfg{SomePlugin}{Setting1} } ) );
    $this->assert_str_equals( 'Value2',
        $vhost->run( sub { $Foswiki::cfg{SomePlugin}{Setting2} } ) );
    $this->assert_str_equals( 'Unchanged',
        $vhost->run( sub { $Foswiki::cfg{SomePlugin}{UnchangedSetting} } ) );
}
...
sub test_set_arbitrary_configurations_with_nested_hashes {
    ...
    $this->assert_equals( 1,
        $vhost->run( sub { $Foswiki::cfg{Languages}{pt}{Enabled}; } ) );
    $this->assert_equals( 0,
        $vhost->run( sub { $Foswiki::cfg{Languages}{pt}{SomeSetting}; } ) );
    $this->assert_equals( 0,
        $vhost->run( sub { $Foswiki::cfg{Languages}{fr}{Enabled}; } ) );
    $this->assert_equals( 1,
        $vhost->run( sub { $Foswiki::cfg{Languages}{fr}{SomeSetting}; } ) );
}

sub test_merge_config {
    my $this = shift;
    my $merge =
      \&Foswiki::Contrib::VirtualHostingContrib::VirtualHost::_merge_config;

    $this->assert_deep_equals( {}, &$merge( {}, {} ) );
    $this->assert_deep_equals( { X => 2 }, &$merge( { X => 1 }, { X => 2 } ) );
    $this->assert_deep_equals( { X => { X1 => 1, X2 => 2 } },
        &$merge( { X => { X1 => 0 } }, { X => => { X1 => 1, X2 => 2 } } ) );
    $this->assert_deep_equals( { X => 1 }, &$merge( 1, { X => 1 } ) );
    $this->assert_deep_equals( 1, &$merge( { X => 1 }, 1 ) );

    my %global = ();
    $global{Languages}{pt}{Enabled} = 0;
    my %local = ();
    $local{Languages}{pt}{Enabled} = 1;
