...
BEGIN {
    if ( !$Foswiki::cfg{VirtualHostingContrib}{VirtualHostsDir} ) {
        ...
        $path =~ /(.*)$/;
        $path = $1;    # untaint, we trust Foswiki configuration
...
sub find {
    ...
    if ( !$class->exists($hostname) ) {
        ...
        $hostname =
          $Foswiki::cfg{VirtualHostingContrib}{ServerAlias}{$hostname};
        return undef unless defined $hostname;
    }

    my $DataDir =
      $Foswiki::cfg{VirtualHostingContrib}{VirtualHostsDir} . "/$hostname/data";
    my $WorkingDir =
      $Foswiki::cfg{VirtualHostingContrib}{VirtualHostsDir}
      . "/$hostname/working";
    my $PubDir =
      $Foswiki::cfg{VirtualHostingContrib}{VirtualHostsDir} . "/$hostname/pub";
    my $self = {
        hostname  => $hostname,
        directory => $Foswiki::cfg{VirtualHostingContrib}{VirtualHostsDir}
          . "/$hostname",
        config => {
            DataDir        => $DataDir,
            PubDir         => $PubDir,
            ...
            Htpasswd => { FileName => "$DataDir/.htpasswd", },
            Log      => { Dir      => $WorkingDir . "/logs", },
            Cache    => {
                RootDir => $WorkingDir . "/cache",
                DBFile  => $WorkingDir . "/cache/foswiki_db",
            },
            RCS         => { WorkAreaDir => "$WorkingDir/work_areas", },
...
sub exists {
    my ( $class, $hostname ) = @_;
    return -d $Foswiki::cfg{VirtualHostingContrib}{VirtualHostsDir}
      . "/$hostname/data";
}
...
sub run {
    ...
    local $Foswiki::Contrib::VirtualHostingContrib::VirtualHost::CURRENT =
      $self->hostname;

    local @Foswiki::cfg{ keys %{ $self->{config} } } =
      map { _merge_config( $Foswiki::cfg{$_}, $self->{config}->{$_} ) }
      ( keys %{ $self->{config} } );
...
sub run_on_each {
    ...
    my @hostnames =
      map { basename $_}
      glob( $Foswiki::cfg{VirtualHostingContrib}{VirtualHostsDir} . '/*' );
